...
- **ユーザーを頼る**: 何回か試行して、上手くいかないときは問題と、現在のステータスと現在までわかっていることを整理して、教えてください
...
- ユーザーから修正を受けたら必ずプロジェクトドキュメントにそのパターンを記録する
...
1. **まず計画を立てる**：チェック可能な項目として計画書を書く
2. **計画を確認する**：実装を開始する前に確認する
3. **進捗を記録する**：完了した項目を随時マークしていく
4. **変更を説明する**：各ステップで高レベルのサマリーを提供する
5. **結果をドキュメント化する**：計画書にレビューセクションを追加する
6. **学びを記録する**：修正を受けた後にプロジェクトドキュメントを更新する
